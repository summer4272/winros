...
$logDir = Join-Path $root "runs\g1_hurdles_run_v1"
$pidFile = Join-Path $logDir "g1_hurdles_run_v1_jobs.csv"
...
$name = "unitree_g1_hurdles_run_v1"
$task = "Unitree-G1-HurdlesRunV1"
...
if (-not $NoResume -and (Test-Path $g1LogRoot)) {
  $sourceRun = Get-ChildItem -Path $g1LogRoot -Directory |
    Where-Object {
      $_.Name -match "_unitree_g1_hurdles_run_v1_" -or
      $_.Name -match "transfer_g1_fast_run_to_hurdles_step_clearance_seed" -or
      $_.Name -match "transfer_g1_fast_run_to_hurdles_v1_seed"
    } |
...
Write-Host "Started G1 hurdles-run V1 job. PID file: $pidFile"
if ($resumeFrom -ne "") {
